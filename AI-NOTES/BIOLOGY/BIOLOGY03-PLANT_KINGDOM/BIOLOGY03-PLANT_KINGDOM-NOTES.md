...
mindmap
  root((PLANT KINGDOM))
    Classification Basis
      Plant body organisation
      Cell wall composition
      Nutrition mode
      Reproductive structures
      Presence of embryo formation
    Algae
      Chlorophyceae
        Green colour - chlorophyll a and b
        Cellulose cell wall
        Starch as food reserve
        Chlamydomonas Volvox Ulothrix Spirogyra Chara
      Phaeophyceae
        Brown colour - fucoxanthin
        Mannitol and laminarin storage
        Holdfast stipe frond
        Ectocarpus Dictyota Laminaria Sargassum Fucus
      Rhodophyceae
        Red colour - r-phycoerythrin
    ...
    Bryophytes
      ...
      Gametophyte dominant and independent
    ...
    Pteridophytes
      ...
      Sporophyte dominant differentiated
      Homosporous and Heterosporous
      Selaginella Pteris Equisetum Lycopodium
      Seed habit origin in heterosporous forms
    Gymnosperms
      Naked seeds - no fruit formation
      Heterosporous
      Pinus Cycas Cedrus
      Pollen chamber direct pollination
    Angiosperms
      Flowering plants
    ...
    Alternation of Generations
      ...
      Haplodiplontic Life Cycle
        Both generations multicellular
```
...
```mermaid
flowchart TD
    Plantae[KINGDOM PLANTAE] --> Crypto["Cryptogams<br/>(non-seed-bearing)"]
    Plantae --> Phanero["Phanerogams<br/>(seed-bearing)"]
    Crypto --> Algae
    Crypto --> Bryo[Bryophytes]
    Crypto --> Ptero[Pteridophytes]
    Phanero --> Gymno[Gymnosperms]
    Phanero --> Angio[Angiosperms]
...
```mermaid
flowchart TD
    A[Algal Reproduction] --> V["Vegetative<br/>Fragmentation"]
    A --> Asex["Asexual<br/>Zoospores (flagellated, motile)"]
    A --> Sex[Sexual — via 3 types of Fusing Gametes]
    Sex --> Iso["Isogamous<br/>Morphologically & physiologically similar gametes"]
    Sex --> Aniso["Anisogamous<br/>Gametes dissimilar in size"]
    Sex --> Oo["Oogamous<br/>One large non-motile female + one small motile male gamete"]

    style A fill:#dbeafe,stroke:#3b82f6,stroke-width:2px
    style V fill:#e2e8f0,stroke:#64748b,stroke-width:2px
    style Asex fill:#fef08a,stroke:#eab308,stroke-width:2px
    style Sex fill:#fef08a,stroke:#eab308,stroke-width:2px
    style Iso fill:#bbf7d0,stroke:#22c55e,stroke-width:2px
    style Aniso fill:#bbf7d0,stroke:#22c55e,stroke-width:2px
    style Oo fill:#86efac,stroke:#16a34a,stroke-width:2px
```
...
```mermaid
flowchart TD
    Gameto["Haploid Gametophyte<br/>(dominant, independent)"] --> Anth["Antheridium<br/>(male organ)"]
    Gameto --> Arch["Archegonium<br/>(female organ)"]
    Anth -->|"Water needed for<br/>antherozoid transfer"| Fert["Fertilisation"]
    Arch --> Fert
    Fert --> Zygote["Diploid Zygote"]
    Zygote --> Sporo["Sporophyte<br/>(dependent on gametophyte)"]
    Sporo -->|Meiosis in spore mother cells| Spores["Haploid Spores"]
    Spores --> Gameto

    style Gameto fill:#86efac,stroke:#16a34a,stroke-width:2px
    style Anth fill:#fef08a,stroke:#eab308,stroke-width:2px
    style Arch fill:#fef08a,stroke:#eab308,stroke-width:2px
    style Fert fill:#dbeafe,stroke:#3b82f6,stroke-width:2px
    style Zygote fill:#fecaca,stroke:#ef4444,stroke-width:2px
    style Sporo fill:#e2e8f0,stroke:#64748b,stroke-width:2px
    style Spores fill:#bbf7d0,stroke:#22c55e,stroke-width:2px
```
...
```mermaid
flowchart LR
    Spore["Haploid Spores"] --> Germ["Germinate into<br/>Gametophyte (Prothallus)"]
    Germ --> Sex["Bears Antheridia & Archegonia"]
    Sex -->|Water-dependent fertilisation| Zyg["Diploid Zygote"]
    Zyg --> SporoDom["Dominant, Independent Sporophyte<br/>(true root/stem/leaves)"]
    SporoDom -->|Meiosis in sporangia| Spore

    style Spore fill:#86efac,stroke:#16a34a,stroke-width:2px
    style Germ fill:#fef08a,stroke:#eab308,stroke-width:2px
    style Sex fill:#fef08a,stroke:#eab308,stroke-width:2px
    style Zyg fill:#fecaca,stroke:#ef4444,stroke-width:2px
    style SporoDom fill:#dbeafe,stroke:#3b82f6,stroke-width:2px
```
...
```mermaid
flowchart TD
    LC[Life Cycle Patterns] --> Haplo["Haplontic<br/>Gametophyte (n) dominant, photosynthetic, free-living<br/>Sporophyte represented only by the zygote"]
    LC --> Diplo["Diplontic<br/>Sporophyte (2n) dominant, photosynthetic, free-living<br/>Gametophyte reduced to few haploid cells"]
    LC --> HaploDiplo["Haplodiplontic<br/>Both gametophyte AND sporophyte multicellular<br/>One phase may be dependent on the other"]

    style LC fill:#dbeafe,stroke:#3b82f6,stroke-width:2px
    style Haplo fill:#fef08a,stroke:#eab308,stroke-width:2px
    style Diplo fill:#86efac,stroke:#16a34a,stroke-width:2px
    style HaploDiplo fill:#bbf7d0,stroke:#22c55e,stroke-width:2px
```
